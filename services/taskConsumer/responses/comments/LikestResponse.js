	...
	async process({ postLink, commentsCount, serviceCredentials: { login, password } }) {
		...
		try {
			...
			await page.waitForFunction(
				() => !!document.querySelector('#hpoints-comments-add-form--2'),
			);
			...
			await page.waitForFunction(
				() => document.querySelectorAll('#comments-fieldset-wrapper textarea').length > 1,
			);

			await page.click('#edit-actions-form2-continue');
			await page.waitForFunction(
				() => !!document.querySelector('#hpoints-comments-add-form--3'),
			);

			await page.evaluate(
				(_postLink, _commentsCount) => {
			...
			await page.waitForFunction(() => {
				const errors = document.querySelectorAll(
					'#hpoints-comments-add-form--3 .messages.error',
				);
				if (errors.length) {
					return true;
				}
			...
			const errors = await page.evaluate(() => {
				const errorsElements = [
					...document.querySelectorAll('#hpoints-comments-add-form--3 .messages.error'),
				];

...
import { groupForVkApiByHref, groupIdForApi, getRandom } from './helper';
...
class VkApi {
	 ...
	 */
	constructor(captcha, logger, token, { version = 5.8, timeout = 5000 } = {}) {
	...
	async groupByHref(href) {
		...
		const {
			response: [group],
		} = await this.apiRequest('groups.getById', { group_id: groupId });
	...
	async getPosts({ publicId, count = 3 }) {
		const {
			response: { items },
		} = await this.apiRequest('wall.get', {

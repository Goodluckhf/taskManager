...
const formatErrorStack = (stack, depth) => {
	const stackArray = stack.split('\n').slice(1).map((stackItem) => {
		return stackItem
			.replace(workDir, './')
			.replace(/^\s*/, `${buildIndentString(depth + 1)}`);
	});
...
const formatError = (error, depth) => {
	...
	return  `${keyValuesString}` +
			...
			`${colorConfig.object.key('stack', 1)}: ${formatErrorStack(error.stack, depth + 1)}`;
};
...
const formatObject = (object, depth = 0) => {
	if (depth === maxDepth) {
		return colorConfig.object.system('Max depth');
	}
	...
	return Object.keys(object).reduce((str, key) => {
		...
		if (Array.isArray(value)) {
			...
			displayString = colorConfig.object.system('null');
		} else if (typeof value === 'undefined') {
			displayString = colorConfig.object.system('undefined');
		} else if (typeof value === 'function') {
			displayString = colorConfig.object.system('Function');

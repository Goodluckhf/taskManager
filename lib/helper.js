 ...
 * @return {string}
 */
export const postLinkByGroup = (group) => {
	return `https://vk.com/wall${group.owner_id}_${group.id}`;
};
 ...
 * @return {String}
 */
export const postIdByLink = link => (
	link.replace('https://vk.com/wall', '')
);
...
export const getRandomNumberString = (size) => {
	return Array.from({ length: size }).reduce(str => (
		`${str}${getRandom(0, 9)}`
	), '');
};
...
export const cleanLink = (_link) => {
	...
	if (/^https?:\/\/vk\.com\/away\.php/.test(_link)) {
